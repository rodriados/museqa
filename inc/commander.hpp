...
namespace msa
{
    ...
    class commander
    {
        ...
        protected:
            std::map<code, std::string> parsed;         /// The list of parsed option results.
            std::vector<std::string> posargs;           /// The list of positional arguments.

        public:
            /**
             * Initializes a new commander instance. As a command line parser and
             * manager, there's nothing else to do at startup besides... parsing!
             * @param options The list of available options to be parsed.
             * @param argc The number of command line arguments.
             * @param argv The command line arguments.
             */
            inline explicit commander(const std::vector<option>& options, int argc, char **argv)
            {
            ...
            {
                const auto& it = parsed.find(opcode);
                return it != parsed.end() && !it->second.empty();
            }
            ...
            {
                return has(opcode) ? convert<T>(parsed.at(opcode)) : fallback;
            }
            ...
            {
                return posargs.size() > size_t(offset) ? convert<T>(posargs[offset]) : fallback;
            }
