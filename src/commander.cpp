...
namespace msa
{
     ...
     */
    static auto prepare(const std::vector<commander::option>& options) noexcept
    -> std::map<std::string, commander::option>
    {
        std::map<std::string, commander::option> opmap;
        
        for(const commander::option& current : options)
     ...
     */
    static auto find(const std::map<std::string, commander::option>& opmap, const std::string& needle) noexcept
    -> const commander::option&
    {
        static commander::option unknown {};
     ...
     */
    auto commander::parse(const std::vector<commander::option>& options, int argc, char **argv) -> void
    {
        const auto opmap = prepare(options);

        for(int i = 1; i < argc; ++i) {
            const commander::option& current = find(opmap, argv[i]);
